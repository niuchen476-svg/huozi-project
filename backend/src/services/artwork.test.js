...
test("绘画提示词包含主题、玩家表达、碎片含义和署名留白要求", async () => {
  const input = await normalizeArtworkInput({
    themeId: "arrival",
  ...
  assert.match(prompt, /每一次抵达都有代价/);
  assert.match(prompt, /湘江渡江路线/);
  assert.match(prompt, /右下角只保留/);
  assert.match(prompt, /不得出现现代武器/);
});

test("AIHubMix 请求格式使用 OpenAI 兼容生图接口且不自动重试", async () => {
  let requestUrl;
  let requestBody;
  ...
  assert.equal(requestUrl, getImageGenerationUrl());
  ...
  assert.match(requestBody.negative_prompt, /日期/);
