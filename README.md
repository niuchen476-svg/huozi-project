...
supabase/functions/_shared/experience-data.ts
supabase/functions/expression/data/experience-data.ts
```
...
GitHub Pages 静态模式下不使用 Express 后端，而是调用 Supabase Edge Function。改动会影响 AI 表达的关卡数据后，需要同步数据并重新部署函数：

```bash
npm run sync-data
npm run deploy-expression
```
...
```text
本地：http://localhost:3001/api/ai/status
线上：https://pfkamgzktfwfotirlocd.supabase.co/functions/v1/expression
线上：https://pfkamgzktfwfotirlocd.supabase.co/functions/v1/artwork
```
