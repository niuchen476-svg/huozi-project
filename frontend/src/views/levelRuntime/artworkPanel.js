...
export function createArtworkRequest({ expression = {}, expressionPayload = {}, playerName = "" } = {}) {
  return {
...
class LevelArtworkPanel {
  ...
  async generate() {
    ...
    try {
      ...
      this.status.textContent = `${error.message || "在线画作暂时无法生成"}。已保留固定纪念画面，不影响完成体验。`;
